...
select lives_ok($$select app.get_settings_workspace_v3()$$, 'beheerder op AAL2 kan instellingen openen');
select is(app.get_settings_workspace_v3()->'settings'->>'clubName', 'Duindorp SV', 'clubnaam is canoniek vast');
select is(jsonb_array_length(app.get_settings_workspace_v3()->'roles'), 3, 'workspace bevat exact drie rollen');
select ok(jsonb_array_length(app.get_settings_workspace_v3()->'seasons') >= 3, 'workspace bevat seed- en testseizoenen');
select is(jsonb_array_length(app.get_settings_workspace_v3()->'staff'), 4, 'workspace bevat staff-profielen');
...
select is((select contact_email from app.app_settings where id), 'kleding@duindorpsv.nl', 'contactmail blijft de gepubliceerde waarde');
select is((select pickup_location from app.app_settings where id), 'Free-Kick Sport, De Savornin Lohmanplein 45, 2566 AE Den Haag', 'afhaallocatie blijft de gepubliceerde waarde');
...
select ok(not exists(select 1 from app.audit_logs where action='settings.updated' and metadata::text ~* 'duindorpsv|savornin'), 'auditmetadata bevat geen contactgegevens of locatie');
select throws_ok($$select app.update_settings_v2('geen-email','Houtrustlaan 1','2566 ZW','Den Haag',true,'Free-Kick Sport','De Savornin Lohmanplein 45','2566 AE','Den Haag','f1000000-0000-4000-8000-000000000001','[]'::jsonb,false,false,null)$$, '22023', 'SETTINGS_BRANDING_MANAGED_SEPARATELY', 'branding kan niet via settings worden gewijzigd');
select throws_ok($$select app.update_settings_v3('f1000000-0000-4000-8000-000000000002','[]'::jsonb,false,false,null)$$, '22023', 'SETTINGS_OPERATIONAL_VALUES_INVALID', 'gearchiveerd seizoen kan niet actief worden');
...
select throws_ok($$select app.get_settings_workspace_v3()$$, '42501', 'STAFF_AUTHORIZATION_REQUIRED', 'kledingcommissie heeft geen instellingenworkspace');
...
select throws_ok($$select app.get_settings_workspace_v3()$$, '42501', 'STAFF_AUTHORIZATION_REQUIRED', 'instellingen vereisen aantoonbaar AAL2');
