...
export const parentPackageOrderDatabaseSchema = z.object({
  ...
  activePackagePriceCents: z.number().int().nonnegative().max(10_000_000).optional(),
  appliedCreditCents: z.number().int().nonnegative().max(10_000_000).optional(),
  successfulPaymentCents: z.number().int().nonnegative().max(10_000_000).optional(),
  remainingDueCents: z.number().int().nonnegative().max(10_000_000).optional(),
  refundDueCents: z.number().int().nonnegative().max(10_000_000).optional(),
  refundCompletedCents: z.number().int().nonnegative().max(10_000_000).optional(),
  refundOutstandingCents: z.number().int().nonnegative().max(10_000_000).optional(),
  financialReconciliationRequired: z.boolean().optional(),
