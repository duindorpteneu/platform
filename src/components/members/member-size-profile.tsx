...
export function MemberSizeProfile({ memberId, profile: initialProfile }: { memberId: string; profile: Profile | null }) {
  ...
  const changes = useMemo(() => profile?.articles.filter((article) => article.editable && (draft[article.id] ?? "") !== (article.selectedVariantId ?? "")) ?? [], [draft, profile]);
  const releasesReservation = changes.some((article) => article.hasReservation);
    ...
    try {
      const response = await fetch("/api/members/sizes", {
        ...
        body: JSON.stringify({
          ...
          reason,
          requestId,
          sizes: changes.map((article) => ({ articleId: article.id, variantId: draft[article.id] || null, releaseReserved: article.hasReservation })),
        }),
      });
  ...
  return <section className="p-5">
    ...
    {!profile ? <p className="mt-4 rounded-lg bg-amber-50 p-4 text-xs leading-5 text-amber-800">Stel eerst een open actief seizoen in om kledingmaten vast te leggen.</p> : profile.articles.length === 0 ? <p className="mt-4 rounded-lg bg-slate-50 p-4 text-xs leading-5 text-slate-500">Koppel eerst actieve artikelen en maten aan dit seizoen.</p> : <>
      ...
      {changes.length > 0 && <div className="mt-4 space-y-3 rounded-lg border border-brand-100 bg-brand-50/40 p-3"><label className="block text-[10px] font-bold uppercase tracking-[0.08em] text-slate-500">Reden<textarea rows={2} minLength={3} maxLength={500} value={reason} onChange={(event) => { setReason(event.target.value); setNotice(null); }} placeholder="Bijvoorbeeld: gecorrigeerd na passen" className="mt-2 w-full rounded-lg border border-line bg-white px-3 py-2 text-xs font-normal normal-case tracking-normal text-ink outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" /></label>{releasesReservation && <label className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-[10px] leading-4 text-amber-900"><input type="checkbox" checked={releaseConfirmed} onChange={(event) => setReleaseConfirmed(event.target.checked)} className="mt-0.5 size-4" /><span>Ik bevestig dat de bestaande reservering wordt vrijgegeven. De nieuwe maat krijgt pas opnieuw voorraad volgens de wachtlijst.</span></label>}</div>}
