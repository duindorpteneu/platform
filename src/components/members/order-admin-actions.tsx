...
export function OrderAdminActions({ orderId, amountDueCents, paid, qrStatus, pickedLines, canRecordCash }: {
  ...
  async function manageQr(action: "rotate" | "revoke") {
    const requestId = qrRequest?.action === action
      ? qrRequest.id
      : crypto.randomUUID();
    setQrRequest({ action, id: requestId });
    ...
    try {
      const response = await fetch("/api/qr/rotate", {
        ...
        body: JSON.stringify({ orderId, action, reason, requestId }),
      });
      const payload = await response.json() as { error?: string };
  ...
  async function correct() {
    const orderLineIds = [...selected].sort();
    const payloadKey = JSON.stringify({
      orderLineIds,
      reason: correctionReason.trim(),
      targetStatus,
    });
    const requestId = correctionRequest?.payloadKey === payloadKey
      ? correctionRequest.id
      : crypto.randomUUID();
    setCorrectionRequest({ id: requestId, payloadKey });
  ...
  return (
    <section className="p-5">
      ...
      <div className="mt-4 rounded-lg border border-line p-4">
        ...
        <textarea id="qr-reason" value={reason} onChange={(event) => { setReason(event.target.value); setQrRequest(null); }} maxLength={500} rows={2} className="mt-1.5 w-full rounded-lg border border-line px-3 py-2 text-xs outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
      ...
      <div className="mt-4 rounded-lg border border-line p-4">
        ...
        {pickedLines.length === 0 ? <p className="mt-2 text-[11px] leading-5 text-slate-500">Er zijn geen actieve uitgegeven regels die gecorrigeerd kunnen worden.</p> : <>
          <div className="mt-3 space-y-2">{pickedLines.map((line) => <label key={line.id} className="flex min-h-11 cursor-pointer items-center gap-3 rounded-lg bg-slate-50 px-3 py-2 text-xs"><input type="checkbox" checked={selectedSet.has(line.id)} onChange={() => { setCorrectionRequest(null); setSelected((current) => selectedSet.has(line.id) ? current.filter((id) => id !== line.id) : [...current, line.id]); }} className="size-4 rounded border-slate-300 text-brand-700 focus:ring-brand-500" /><span className="font-semibold text-ink">{line.article} · {line.size}</span></label>)}</div>
          <label htmlFor="correction-target" className="mt-3 block text-[11px] font-semibold text-ink">Terugzetten naar</label>
          <select id="correction-target" value={targetStatus} onChange={(event) => { setTargetStatus(event.target.value as typeof targetStatus); setCorrectionRequest(null); }} className="mt-1.5 h-10 w-full rounded-lg border border-line bg-white px-3 text-xs outline-none focus:border-brand-500"><option value="ready_for_pickup">Af te halen — reservering behouden</option><option value="backorder">Nalevering — voorraad vrijgeven</option></select>
          <label htmlFor="correction-reason" className="mt-3 block text-[11px] font-semibold text-ink">Verplichte reden</label>
          <textarea id="correction-reason" value={correctionReason} onChange={(event) => { setCorrectionReason(event.target.value); setCorrectionRequest(null); }} maxLength={500} rows={2} className="mt-1.5 w-full rounded-lg border border-line px-3 py-2 text-xs outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
