...
const refundStatusLabel = {
  due: "Refund gereed",
  requesting: "Refund wordt aangevraagd",
  queued: "Refund in behandeling",
  pending: "Refund in behandeling",
  processing: "Refund in behandeling",
  completed: "Terugbetaald",
  failed: "Refund mislukt",
  canceled: "Refund geannuleerd",
  manual_due: "Handmatige terugbetaling vereist",
  manual_completed: "Handmatig terugbetaald",
  reconciliation_required: "Reconciliatie vereist",
} as const;
...
export function PaymentWorkspace({ workspace }: { workspace: Workspace }) {
  ...
  const [selectedRefund, setSelectedRefund] = useState<Workspace["refunds"][number] | null>(null);
  const [reason, setReason] = useState("");
  const [evidenceReference, setEvidenceReference] = useState("");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const attention = workspace.summary.review + workspace.summary.duplicatePaid
    + workspace.summary.manualRefundRequired + workspace.summary.refundReconciliationRequired;
  async function recordRefund() {
    const source = selectedRefund ?? selected;
    if (!source || reason.trim().length < 4 || evidenceReference.trim().length < 4) return;
    setBusy(true);
    setError(null);
    try {
      const response = await fetch("/api/payments/manual/refund", {
        ...
        body: JSON.stringify({
          ...(selectedRefund ? { refundId: selectedRefund.refundId } : {}),
  ...
  async function retryMollieRefund(refund: Workspace["refunds"][number]) {
    setBusy(true); setError(null);
    try {
      const response = await fetch("/api/payments/mollie/refund", {
        method: "POST",
        headers: { "content-type": "application/json", "x-duindorp-csrf": "same-origin" },
        body: JSON.stringify({ refundId: refund.refundId, requestId: refund.refundId }),
      });
      const payload = await response.json() as { error?: string };
      if (!response.ok) throw new Error(payload.error ?? "De Mollie-refund kon niet worden gestart.");
      router.refresh();
    } catch (refundError) {
      setError(refundError instanceof Error ? refundError.message : "De Mollie-refund kon niet worden gestart.");
  ...
  return <div className="mx-auto max-w-[1400px]">
    ...
    {workspace.refunds.length > 0 && <section className="mt-6 overflow-hidden rounded-xl border border-line bg-white shadow-card">
      <div className="border-b border-line px-6 py-5"><h2 className="text-base font-bold text-brand-900">Pakketcorrecties en terugbetalingen</h2><p className="mt-1 text-xs text-slate-500">Providerstatus en handmatige verplichtingen blijven los van de historische betaling zichtbaar.</p></div>
      <div className="divide-y divide-line">{workspace.refunds.map((refund) => <div key={refund.refundId} className="flex flex-col gap-3 px-6 py-4 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1"><p className="text-xs font-bold text-brand-900">{refund.memberName} · {refund.fromPackage} → {refund.toPackage}</p><p className="mt-1 text-[11px] text-slate-500">{money.format(refund.amountCents / 100)} · {methodLabel[refund.method]} · {refundStatusLabel[refund.status]}</p></div>
        {refund.status === "manual_due" && workspace.canRecordRefund && <button type="button" onClick={() => { setSelectedRefund(refund); setSelected(null); setError(null); }} className="rounded-lg border border-brand-200 px-3 py-2 text-[11px] font-bold text-brand-700">Terugbetaling registreren</button>}
        {refund.method === "mollie" && ["due", "failed"].includes(refund.status) && workspace.canRecordRefund && <button type="button" disabled={busy || (refund.status === "failed" && !refund.retryable)} onClick={() => void retryMollieRefund(refund)} className="rounded-lg bg-brand-700 px-3 py-2 text-[11px] font-bold text-white disabled:opacity-50">Mollie-refund starten</button>}
      </div>)}</div>
    </section>}
    {(selected || selectedRefund) && <section aria-labelledby="manual-refund-heading" className="mt-6 rounded-xl border border-amber-200 bg-white p-6 shadow-card">
      <h2 id="manual-refund-heading" className="text-base font-bold text-brand-900">Externe terugbetaling registreren</h2>
      <p className="mt-2 max-w-3xl text-xs leading-5 text-slate-500">Deze handeling registreert een reeds extern uitgevoerde terugbetaling. De applicatie draagt hierbij geen geld over en verandert de historische betaling, voorraad of QR-status niet.</p>
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <div className="rounded-lg bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Terugbetaling</p><p className="mt-2 text-sm font-bold text-brand-900">{(selectedRefund ?? selected)!.memberName} · {money.format((selectedRefund ?? selected)!.amountCents / 100)} · {methodLabel[(selectedRefund ?? selected)!.method]}</p></div>
      ...
      <div className="mt-5 flex flex-wrap justify-end gap-3"><button type="button" onClick={() => { setSelected(null); setSelectedRefund(null); setError(null); }} disabled={busy} className="h-10 rounded-lg border border-line px-4 text-xs font-bold text-slate-600">Annuleren</button><button type="button" onClick={() => void recordRefund()} disabled={busy || reason.trim().length < 4 || evidenceReference.trim().length < 4} className="inline-flex h-10 items-center gap-2 rounded-lg bg-brand-700 px-4 text-xs font-bold text-white disabled:opacity-50">{busy ? <Loader2 className="size-4 animate-spin" /> : <RotateCcw className="size-4" />}Externe terugbetaling registreren</button></div>
    </section>}
  </div>;
}
