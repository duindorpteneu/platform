...
function VariantManager({ article, selected, selectedId, saving, onSelect, onSave }: { article: Article; selected: Variant | null; selectedId: string | "new"; saving: boolean; onSelect: (id: string | "new") => void; onSave: (body: unknown, message: string) => void }) {
  ...
  const [sortOrder, setSortOrder] = useState(0);
  const [active, setActive] = useState(true);
  useEffect(() => { setSize(selected?.size ?? ""); setSupplierCode(selected?.supplierCode ?? ""); setAliases(selected?.aliases.join(", ") ?? ""); setSortOrder(selected?.sortOrder ?? article.variants.length * 10); setActive(selected?.active ?? true); }, [selected, article]);
  const parsedAliases = aliases.split(/[,;\n]/u).map((value) => value.trim()).filter(Boolean);
  function submit(event: FormEvent) { event.preventDefault(); onSave({ articleId: article.id, variantId: selected?.id ?? null, size, supplierCode, aliases: parsedAliases, active, sortOrder }, selected ? "Variant en maatalias(sen) bijgewerkt." : "Variant toegevoegd."); }
  function inactivate(item: Variant) { onSave({ articleId: article.id, variantId: item.id, size: item.size, supplierCode: item.supplierCode, aliases: item.aliases, active: false, sortOrder: item.sortOrder }, "Variant geïnactiveerd; historische regels blijven behouden."); }
