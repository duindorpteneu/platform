...
import type { ReleaseControlWorkspace } from "@/lib/release-control-contract";
import { ReleaseControlPanel } from "@/components/settings/release-control-panel";
import { SupplierAccessPanel } from "@/components/settings/supplier-access-panel";
...
export function SettingsWorkspace({
  ...
  workspace: Workspace;
  releaseControls: ReleaseControlWorkspace;
}) {
  ...
  async function createSeason() {
    ...
    if (!seasonDraft.name.trim() || defaultAmountCents === null || (seasonDraft.startsOn && seasonDraft.endsOn && seasonDraft.startsOn > seasonDraft.endsOn)) {
      setNotice({ tone: "error", text: "Controleer de seizoensnaam, datums en het legacybedrag." });
      return;
    }
  ...
  return <div className="mx-auto max-w-[1400px]">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-brand-500">Beheer en beveiliging</p><h1 className="mt-2 text-[30px] font-bold tracking-[-0.04em] text-brand-900 md:text-[34px]">Instellingen</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Beheer de vaste clubcontext, seizoenen, operationele veiligheidsschakelaars en medewerkers. Nieuwe pakketprijzen beheer je uitsluitend bij Kledingpakketten.</p></div>
    ...
    <div className="mt-6 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_430px]">
      <form onSubmit={saveSettings} className="space-y-6">
        ...
        <section className="rounded-xl border border-line bg-white p-6 shadow-card">
          <div className="flex flex-col gap-3 border-b border-line pb-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-brand-500">Clubprofiel</p><h2 className="mt-1 text-lg font-bold text-brand-900">Gepubliceerde clubbranding</h2><p className="mt-1 text-xs leading-5 text-slate-500">Contact en adressen vormen één versie met e-mail, portaal, QR en uitgifte.</p></div><Link href="/backoffice/emails?tab=branding" className="inline-flex h-9 shrink-0 items-center justify-center rounded-lg border border-brand-200 px-3 text-xs font-bold text-brand-700 hover:bg-brand-50">Branding beheren</Link></div>
          <dl className="mt-5 grid gap-4 text-xs md:grid-cols-2"><div className="rounded-xl border border-line bg-slate-50 p-4"><dt className="font-bold text-brand-900">Vereniging en contact</dt><dd className="mt-2 leading-5 text-slate-600">{currentWorkspace.settings.clubName}<br />{currentWorkspace.settings.clubAddressLine}<br />{currentWorkspace.settings.clubPostalCode} {currentWorkspace.settings.clubCity}<br />{currentWorkspace.settings.contactEmail}</dd></div><div className="rounded-xl border border-brand-100 bg-brand-50/40 p-4"><dt className="flex items-center gap-2 font-bold text-brand-900"><MapPin className="size-4" />Afhaallocatie</dt><dd className="mt-2 leading-5 text-slate-600">{currentWorkspace.settings.pickupName}<br />{currentWorkspace.settings.pickupAddressLine}<br />{currentWorkspace.settings.pickupPostalCode} {currentWorkspace.settings.pickupCity}</dd></div></dl>
          <p className="mt-4 text-[10px] text-slate-500">Gepubliceerde brandingrevisie {currentWorkspace.settings.brandingRevision} · integriteitscode {currentWorkspace.settings.brandingContentHash.slice(0, 12)}</p>
        </section>

        <section className="rounded-xl border border-line bg-white p-6 shadow-card">
          <div className="border-b border-line pb-5"><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-brand-500">Seizoenen</p><h2 className="mt-1 text-lg font-bold text-brand-900">Actief seizoen en legacycompatibiliteit</h2><p className="mt-1 text-xs text-slate-500">Het legacybedrag geldt alleen voor historische losse bestellingen. Nieuwe pakketorders gebruiken altijd de immutable prijs van de gepubliceerde pakket­revisie.</p></div>
          {currentWorkspace.seasons.length === 0 ? <p className="mt-5 rounded-lg bg-amber-50 p-4 text-xs text-amber-800">Er is nog geen seizoen beschikbaar. Voeg hieronder het eerste seizoen toe.</p> : <><label className="mt-5 block text-xs font-semibold text-ink">Actief open seizoen<select className={fieldClass} value={activeSeasonId} onChange={(event) => setActiveSeasonId(event.target.value)}><option value="">Geen actief seizoen</option>{currentWorkspace.seasons.filter((season) => season.status === "open").map((season) => <option key={season.id} value={season.id}>{season.name}</option>)}</select></label><div className="mt-5 divide-y divide-line rounded-xl border border-line">{currentWorkspace.seasons.map((season) => <div key={season.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold text-brand-900">{season.name}</p><p className="mt-1 text-[10px] text-slate-400">{season.status === "open" ? "Open" : "Gearchiveerd"}{season.active ? " · Actief" : ""}{season.startsOn ? ` · vanaf ${season.startsOn}` : ""}</p></div><label className="text-xs font-semibold text-ink">Legacybedrag losse order<div className="relative mt-2"><span className="absolute left-3 top-2.5 text-sm text-slate-400">€</span><input className="h-10 w-40 rounded-lg border border-line pl-8 pr-3 text-right text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" inputMode="decimal" value={amounts[season.id] ?? ""} onChange={(event) => setAmounts((current) => ({ ...current, [season.id]: event.target.value }))} aria-label={`Legacybedrag losse bestelling ${season.name}`} /></div></label></div>)}</div></>}
        ...
        <ReleaseControlPanel workspace={releaseControls} />
      </form>
