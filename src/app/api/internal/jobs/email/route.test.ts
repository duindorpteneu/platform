...
describe("POST /api/internal/jobs/email", () => {
  beforeEach(() => {
    ...
    mocks.rpc.mockReset().mockImplementation((name: string, args: Record<string, unknown>) => {
      if (name === "claim_email_jobs") return Promise.resolve({ data: { claimToken: args.p_claim_token, jobs: [job] }, error: null });
  ...
  it("does no work when the runledger cannot be started", async () => {
    ...
    expect(mocks.rpc).not.toHaveBeenCalledWith("claim_email_jobs", expect.anything());
    expect(mocks.send).not.toHaveBeenCalled();
  });
});
