...
export async function GET() {
  ...
  const { data, error } = await admin.rpc("get_parent_candidates", { p_token_hash: session.tokenHash });
  if (error) return NextResponse.json({ error: "Koppelbare leden konden niet worden geladen." }, { status: 503 });
