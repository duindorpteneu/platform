...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof Error && error.message === "EMAIL_PORTAL_URL_REQUIRED") return NextResponse.json({ error: "De toegangstemplate moet {{portaal_url}} bevatten." }, { status: 400 });
