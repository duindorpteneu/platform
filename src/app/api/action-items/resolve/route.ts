...
import { actionItemResolveRequestSchema } from "@/lib/action-item-contract";
import {
  actionItemError,
...
export async function POST(request: Request) {
  ...
  const parsed = actionItemResolveRequestSchema.safeParse(body.data);
  if (!parsed.success) {
    return actionItemError("Een geldige oplossing en reden zijn verplicht.", 400);
  }

  try {
    const result = await mutateActionItem(
      { operation: "resolve", input: parsed.data },
