...
import { actionItemDismissRequestSchema } from "@/lib/action-item-contract";
import {
  actionItemError,
...
export async function POST(request: Request) {
  ...
  const parsed = actionItemDismissRequestSchema.safeParse(body.data);
  if (!parsed.success) {
    return actionItemError("Een afwijsreden van minimaal drie tekens is verplicht.", 400);
  }

  try {
    const result = await mutateActionItem(
      { operation: "dismiss", input: parsed.data },
