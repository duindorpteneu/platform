...
import { actionItemStartRequestSchema } from "@/lib/action-item-contract";
import {
  actionItemError,
...
export async function POST(request: Request) {
  ...
  const parsed = actionItemStartRequestSchema.safeParse(body.data);
  if (!parsed.success) {
    return actionItemError("Ongeldige starthandeling.", 400);
  }

  try {
    const result = await mutateActionItem(
      { operation: "start", input: parsed.data },
