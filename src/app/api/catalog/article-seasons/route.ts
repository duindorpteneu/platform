...
export async function POST(request: Request) {
  const guarded = guardBrowserMutation(request, { body: { allowedContentTypes: ["application/json"], maxBytes: 40_000 } });
  ...
  try {
    ...
    const parsed = bulkArticleSeasonRequestSchema.safeParse(body);
