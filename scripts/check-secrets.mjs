...
const sensitiveAssignment = new RegExp(
  String.raw`^\s*(?:export\s+)?(?:SUPABASE_SECRET_KEY|PARENT_TOKEN_PEPPER|CRON_SECRET|IMPORT_STAGING_ENCRYPTION_KEY|OPERATIONS_HEARTBEAT_URL|MOLLIE_API_KEY|SENDGRID_API_KEY|SENDGRID_EVENT_WEBHOOK_PUBLIC_KEY|DATABASE_URL|POSTGRES_PASSWORD|CLIENT_SECRET|PRIVATE_KEY)\s*=\s*(.+?)\s*$`,
  "i",
);
