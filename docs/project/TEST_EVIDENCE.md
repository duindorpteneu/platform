...
- Beide verificatieroutes binden nu expliciet `.schema("app")` vóór de v4-RPC. Een echte businessafwijzing blijft 401; een RPC- of resultaatschemafout is 503 en wordt in hosted evidence PII-vrij als `CODE_CONSUMPTION_UNAVAILABLE` onderscheiden. Gerichte route- en harnastests, TypeScript en ESLint zijn groen; protected CI, redeploy en hosted parent-loginacceptatie blijven vereist.
