...
- `pnpm test:db:parent-otp-delivery-gap-upgrade` bewijst een niet-lege historische upgrade met een verlopen én een actieve poging: alleen de verlopen poging wordt gereconcilieerd, de actieve blijft in v14/v15 fail-closed en kan daarna haar echte providerresultaat vastleggen. Vervolgens detecteren beide healthversies een nieuwe open gap en een nieuwe delivery-uncertainty. Volledige lokale regressie: 225 Vitestbestanden/1.384 tests, 60 pgTAP-bestanden/1.997 assertions, lint, TypeScript, workflowlint en productiebuild groen.
