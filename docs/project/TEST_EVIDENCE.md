...
- `scripts/providers/verify-sendgrid-fingerprints.test.ts` bewijst de ingestelde globale/EU-API-host, appkeyfingerprint, exact minimale `mail.send`-scope, exact minimale adminkeyscopes, strikte adminkeyaccountidentiteit, provider-HTTP-redactie en fail-closed afwijzing van afwijkende host, identiteit, scope of fingerprint. De handmatige workflow krijgt stagingsecrets pas na de herbruikbare actuele-main/exact-artifactpreflight, herverifieert de publieke live SHA/digest en schrijft geen fingerprint of provideridentiteit naar logs. De appkey-adminkey-accountrelatie wordt niet uit losse hashes afgeleid maar blijft onderdeel van de echte send/inbox/signed-eventacceptatie.
