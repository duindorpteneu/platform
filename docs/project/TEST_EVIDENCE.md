...
- Alle 19 databaseconcurrencyharnassen zijn groen, inclusief gezins-e-mailtransfer, oudertoegang, OTP, mailprojectie/-supersession, import, voorraad, betaling en refund. Phase-B- en inventory-upgradereconciliaties, de Mollie-fixture, stagingcleanup van inmiddels 115 operationele tabellen en capaciteit op 1.500 leden/10.000 orderregels zijn eveneens groen.
...
- Definitieve lokale regressies na reviewherstel: schone migratiereplay groen; 60 pgTAP-bestanden/1.990 assertions groen; 225 Vitestbestanden/1.382 tests groen. Het echte OTP-concurrencyharnas bewijst daarnaast supportrequest-idempotentie, lock-before-rate-limit en exact één afleverpoging bij parallelle replay.
...
- De PR-reviewcorrecties bewijzen dat `forceNew` alleen cookiegebonden en als atomische compare-and-rotate op exact het verwachte challenge-ID werkt; een stale cookie kan een nieuwere challenge niet sluiten. De stagingharness start onder de workflow-Node-versie met TypeScript-stripping en begrensde `@/`-aliasresolutie.
- SMTP 550-554 opent uitsluitend bij bewezen permanente `5.1.x`-adresstatus een suppressie; quota-, policy- en contentafwijzingen niet. SendGrid hard-bouncefeedback blijft suppressie openen, provideracceptatietijd telt uitsluitend echte `accepted`-outcomes en kindprojecties gebruiken alleen actuele actieve grants.
- Deployrun `32496684158` stopte fail-closed vóór applicatievervanging toen een historische recipientbackfill terecht op de immutable OTP-ledgerguard botste. De forward-only brugmigraties `20260821161500` en `20260821162500` openen uitsluitend een `NULL -> recipient_identity_id`-backfill waarbij alle bestaande kolommen gelijk blijven en herstellen direct daarna de volledig strikte guard.
- `pnpm test:db:parent-otp-recipient-upgrade` reproduceert het stagingpad vanaf migratie `20260821161500` met een echte historische afleverpoging: de tijdelijke uitzondering sluit zichzelf reeds op de backfillreconciliatie, de identity wordt gekoppeld, beide reconciliaties zijn `passed` en zelfs een no-op-update wordt na afloop opnieuw met `PARENT_OTP_DELIVERY_LEDGER_IMMUTABLE` geweigerd. Schone replay van alle 172 migraties is eveneens groen.
- Het deploycontract gebruikt zowel voor dry-run als apply expliciet `--include-all`, zodat ingevoegde forward-only herstelmigraties ook op een remote die een latere migratie al veilig toepaste controleerbaar worden meegenomen.
- Deployrun `32503204105` paste de volledige recipienthealthketen succesvol toe, maar activeerde de kandidaat niet: v14 had bij het scheiden van recipient- en systeemfouten per ongeluk de bestaande v13-uitsluitingen vervangen. De kandidaat telde daardoor twee bewuste `eligibility_changed_before_send`-terminals en één exact identificeerbare pre-release reminderfailure mee; rollback bleef op die laatste fail-closed rood.
- Forward migration `20260821170721` behoudt safe-terminalredenen, beide exact geattesteerde historische failureversies én bewezen recipientfailure, terwijl een gewijzigde of nieuwe systeemfailure opnieuw blokkeert. De eenmalige nieuwe erkenning is gebonden aan de vooraf geaudite staging-job-id, aanmaakversie, wijzigingsversie en delivery-attempt-id; een gelijkvormige andere job wordt niet erkend. `pnpm test:db:email-health-v14-upgrade` bewijst het niet-lege upgradepad en de wijzigingsdetectie zonder persoonsgegevens in evidence.
- Nog niet groen te markeren: de na review opnieuw vereiste protected PR-CI, merged-main-CI, Deploy staging en de hosted parent-loginacceptatierun.

## OTP-deliverygap herstel na stagingrun 32510204768 — 2026-08-21

- Exact-main CI-run `32510204860` voor `25b7d0c9e2cd2b91bd72db78abce852bdf7614ba` was volledig groen. Deployrun `32510204768` paste migration `20260821170721` toe; directe databaseverificatie bewees daarna `emailJobs.failed=0` en een exact matchende acknowledgement.
- De kandidaat bleef terecht 503 door historische `parentOtpDelivery.stalePrepared`: immutable attempts waren voorbereid, maar een exceptionpad had geen einduitkomst geappend. De workflow activeerde de kandidaat niet en kon de oude app na de database-forward-roll evenmin gezond bewijzen.
- Migration `20260821183154` vergrendelt attempt- en outcomeledgers in één transactie. Zij appendt uitsluitend voor verlopen outcome-loze pre-v3-pogingen de waarheidsgetrouwe uitkomst `delivery_uncertain`; actieve en mogelijk in-flight pogingen blijven immutable en zichtbaar. De reconciliation bewaart PII-vrije auditregels plus exacte evidence.
- `pnpm test:db:parent-otp-delivery-gap-upgrade` bewijst een niet-lege historische upgrade met een verlopen én een actieve poging: alleen de verlopen poging wordt gereconcilieerd, de actieve blijft in v14/v15 fail-closed en kan daarna haar echte providerresultaat vastleggen. Vervolgens detecteren beide healthversies een nieuwe open gap en een nieuwe delivery-uncertainty. Volledige lokale regressie: 225 Vitestbestanden/1.385 tests, 60 pgTAP-bestanden/1.997 assertions, lint, TypeScript, workflowlint en productiebuild groen.

## Hosted parent-loginacceptatie en clock-skewcorrectie — 2026-08-21

- Exact-main CI `32524089882`, deploy `32524089952`, publieke `/api/live` en `/api/health` zijn groen voor release `eaa2ed349a908cf9b04b5f6dd5b3b34e2c1668c5` en artifact `sha256:1b1c4747befc94c8017f5c61a879f0b232caaf38ad96623fb22c248c86efdbfc`; migration `20260821183154` is toegepast en het PostgREST-contract slaagde.
- Parent-loginacceptatie `32526364729` bond exact dezelfde release en safe-recipientconfiguratie, maar stopte op `CODE_CONSUMPTION_FAILED`. De databasegrens `p_session_expires_at <= statement_timestamp() + 30 days` en de appwaarde `Date.now() + 30 days` bewezen een echte klokskew-race; geen credential of ontvanger verscheen in output.
- Gerichte regressies bewaken de begrensde database-expiry voor code én directe link, de canonieke 30-dagencookie en een vooraf via Postgres vastgelegde cleanupgrens die alleen run-nieuwe challenges sluit. Een nieuwe protected release- en acceptatierun blijft vereist voordat deze hosted gate groen wordt gemarkeerd.
- Exact-main CI `32529328503` en deploy `32529328523` zijn groen voor herstelrelease `dfaa21e551a926d395a310edce8b827ead748d52`; publieke health retourneerde 200 en artifact `sha256:a268e874f811ff885df9e9e0bb06b3f2a60f3cf3fa1d588d1bacd40d33b46f01`. Hosted acceptatie `32531359672` bleef `CODE_CONSUMPTION_FAILED`, waarmee de eerdere expiryhypothese werd verworpen.
...
- Het fixture verwijdert vóór de race uitsluitend het eigen voorbereidende waiting-stock-event, dispatch-, transition- en episoderijen, onder dezelfde tijdelijke replicatriggeruitschakeling als de bestaande fixturecleanup. De daaropvolgende betaling maakt daardoor het event onder test zelf aan; productiecode en append-only runtimegedrag wijzigen niet.
- Tien opeenvolgende volledige runs van `scripts/test-inventory-concurrency.sh` zijn groen. Een aanvullende reviewrun hield het aantal reeds bestaande lokale orphan transitions exact gelijk (`70 -> 70`), waarmee de herstelde cleanup geen nieuwe ledgervervuiling achterlaat. Bij een toekomstige subprocessfout worden de tijdelijke, uitsluitend synthetische PostgreSQL-logs vóór cleanup getoond.

## Staging ouderlogin-RPC-schemabinding — 2026-08-22

- Exact-main CI `32552743476`, stagingdeploy `32552743426` en SendGrid-provideracceptatie `32553915985` zijn groen voor release `fb0888662dd657215d5790f1f78608a39d2301de`; SendGrid bewees applicatieverzending, ondertekende recipient-server delivery, cleanup en finale health.
- Parent-loginacceptatie `32553917452` faalde in twee volledig opgeruimde pogingen deterministisch met `CODE_CONSUMPTION_REJECTED`. De route riep de uitsluitend als `app.consume_parent_login_challenge_v4` gepubliceerde RPC via Supabase' standaardschema `public` aan; route-unittests mockten de schemagrens en konden de echte `PGRST202` daardoor niet zien.
- Beide verificatieroutes binden nu expliciet `.schema("app")` vóór de v4-RPC. Een echte businessafwijzing blijft 401; een RPC- of resultaatschemafout is 503 en wordt in iedere hosted code-, link-, consumed-credential- en replayleg PII-vrij als `PARENT_VERIFICATION_UNAVAILABLE` onderscheiden. Gerichte route- en harnastests, TypeScript en ESLint zijn groen; protected CI, redeploy en hosted parent-loginacceptatie blijven vereist.
