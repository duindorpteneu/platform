...
- Migration `20260821183154` vergrendelt attempt- en outcomeledgers in één transactie. Zij appendt voor iedere outcome-loze pre-v3-poging de waarheidsgetrouwe uitkomst `delivery_uncertain`, zonder de afzonderlijke actieve code/linkchallenge te consumeren, en schrijft PII-vrije auditregels plus exacte evidence in de reconciliation.
- `pnpm test:db:parent-otp-delivery-gap-upgrade` bewijst een niet-lege historische upgrade met een verlopen én een actieve poging, nul verborgen healthblockers in zowel rollback-RPC v14 als kandidaat-RPC v15 en daarna fail-closed detectie door beide versies van zowel een nieuwe open gap als een nieuwe delivery-uncertainty. Volledige lokale regressie: 225 Vitestbestanden/1.384 tests, 60 pgTAP-bestanden/1.997 assertions, lint, TypeScript, workflowlint en productiebuild groen.
