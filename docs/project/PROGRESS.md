...
## Current phase
Fase B is op 2 augustus 2026 expliciet goedgekeurd. Addendum v1.1 legt het pakket-/lid-seizoenmodel, DOB-toegang, dynamische import, selectieve oudergrants, allocatiegebonden QR, scanner-PWA en staging-opschoning vast. Het op 21 augustus 2026 geautoriseerde addendum v1.2 vervangt uitsluitend ouderlogin-, OTP-support- en afleverbewijsregels. Implementatie blijft forward-only; production blijft geblokkeerd.
...
## Ouderloginbetrouwbaarheid en mailcontrole — 2026-08-21

- De product owner heeft bindend addendum v1.2 geautoriseerd: één stabiele tienminutenuitdaging hergebruikt code, directe-inlogproof en deadline; beide methoden consumeren dezelfde challenge. De portaaluitnodiging blijft tokenloos en `login_otp` blijft het enige template.
- De rolgrens is vastgelegd zonder vierde rol: alleen de beheerder mag onder AAL2 OTP-supportmail sturen/resetten; de kledingcommissie ziet uitsluitend gemaskeerde ontvangerstatus. Grant-, e-mail-, sessie- en suppressiebeheer blijft beheerder-only.
- SMTP-acceptatie, bewezen aflevering, recipientproblemen en systemische mailhealth zijn canoniek gescheiden. Recipienthistorie blijft aan het oude genormaliseerde adres bij een gezins-e-mailtransfer.
- Een nieuwe exact-SHA stagingacceptatieworkflow gebruikt de bestaande trusted deploypreflight, veilige smoke-ontvanger, zelfopruimende fixtures, always-cleanup en PII-/credentialvrij attestatiebewijs. De lokale implementatie en regressies zijn afgerond; protected CI, stagingdeploy en hosted acceptatie blijven de releasepoorten.
- Reviewherstel maakt publieke `forceNew` cookie- én challenge-ID-gebonden, supportacties request-idempotent en accountgeserialiseerd, en SMTP-suppressie afhankelijk van bewezen permanente adresstatus. De volledige lokale suite is opnieuw groen met 1.382 applicatietests, 1.990 databaseasserties, concurrency, productiebuild en browseracceptatie.
- De eerste exact-main stagingdeploy stopte vóór appvervanging op de bestaande immutable OTP-ledgertrigger tijdens recipientbackfill. Twee forward-only compatibiliteitsmigraties laten uitsluitend de historische null-identitybinding toe; het venster sluit zichzelf zodra de backfillreconciliatie commit en de volgende migration herstelt de oorspronkelijke functie. De deploy voert gecontroleerd ook ingevoegde oudere migrations uit; een nieuwe upgradegate reproduceert exact dit niet-lege stagingpad.
- De tweede exact-main stagingdeploy paste die migrations succesvol toe, maar v14 maakte bestaande v13-healthuitsluitingen onbedoeld ongedaan. Een volgende forward migration combineert nu safe terminals, exacte historische erkenningen en recipientbewijs zonder nieuwe systeemfouten te verbergen; een productie-achtige upgradefixture bewaakt dit pad in CI.

## OTP-deliverygap herstel en health v15 — 2026-08-21

- Stagingdeploy `32510204768` bevestigde dat e-mailhealth v14 de geaudite mailfailure correct uitsloot, maar legde een tweede historische blocker bloot: voorbereide OTP-deliveryattempts zonder append-only einduitkomst.
...
- De server-side sessie-expiry ligt vijf minuten binnen de databasegrens; de browsercookie behoudt canoniek exact dertig dagen en de databasesessie blijft de autoritatieve toegangsgrens. Always-cleanup gebruikt een vóór de eerste mutatie vastgelegde Postgres-tijdgrens en sluit uitsluitend challenges die daarna door de eigen acceptatierun zijn gemaakt; vooraf bestaande challenges van de veilige smoke-ontvanger blijven onaangetast.
- Herstelde release `dfaa21e551a926d395a310edce8b827ead748d52` passeerde exact-main CI `32529328503`, stagingdeploy `32529328523` en publieke health, maar acceptatie `32531359672` stopte opnieuw vóór consumptie. De veilige algemene smoke-ontvanger had geen gegarandeerde actieve oudergrant; de enumeration-safe publieke route gaf daardoor terecht dezelfde neutrale challengevorm terug.
- Het harnas maakt nu vóór de eerste request een run-uniek synthetisch lid, actieve grant en synthetisch AAL2-beheerprofiel voor uitsluitend de veilige ontvanger. Vóór de publieke request kiest het een random challenge-UUID, legt die duurzaam vast en stuurt hem met een HMAC-proof; uitsluitend de stagingruntime met de expliciete acceptatiegate accepteert zo'n voorstel. Alleen wanneer de route exact die UUID teruggeeft is atomair bewezen dat de run de challenge maakte; iedere bestaande of gelijktijdig gemaakte challenge stopt de proef zonder claim of consumptie. Normale en productieaanvragen blijven server-random. Verificaties krijgen onafhankelijk een staging-only HMAC-correlatie die `consume_parent_login_challenge_v4` atomair op uitsluitend de daardoor gemaakte sessie opslaat; cleanup raakt dus nooit de sessie van een andere browser die dezelfde challenge consumeerde. De tweede challenge ontstaat via de echte AAL2-support-resend en provideradapter, waardoor één poging en één rerun samen binnen de onveranderde publieke vijf-per-uurlimiet blijven. Een gelijktijdige bestaande challenge wordt door resend niet ingetrokken; de immutable afleverpoging wordt wel afgerond en de proef stopt daarna veilig op de afwijkende challenge-ID. De runner ontvangt uitsluitend de staging-providerconfiguratie voor de veilige ontvanger. Alle correlatiedigests zijn met het server-only parentgeheim ge-HMAC't en geen offline raadbare e-mailhash of credential. Een rerun gebruikt voor dezelfde GitHub-run dezelfde fixture-identiteiten en ruimt eventuele persistente voorgangers vóór prepare op. De private safe-recipientaccountidentity, rate-limithistorie en append-only audit/afleverhistorie blijven behouden.

## Voorraadqueue- en Mail-v2-lifecycle — 2026-08-20

- De queuepoison is herleid tot een foutieve classificatie: gewone onvoldoende voorraad werd als concurrentiemutatie herprobeerd, waarna poging tien `queued` bleef maar niet meer werd geselecteerd.
...
- De eerste protected PR-run legde een echte kwadratische allocatiehotspot bloot: de volledige betaalbalans werd na iedere reservering opnieuw voor de hele wachtlijst berekend. Forward migration `20260822013000_effective_payment_unpaid_fast_path.sql` behoudt de canonieke correctie-/refundgrens, sluit positieve orders zonder betaal- of correctiehistorie direct als onbetaald uit en doorloopt de geordende FIFO-kandidaten éénmaal. De volledige 1.500-leden/10.000-regelscapaciteitsproef daalde lokaal van meer dan 6:40 zonder einde naar 2:39 groen, met staff/QR/uitgifte-p95 van 396/169/573 ms.
- De laatste reviewthreads zijn inhoudelijk gesloten: pakketcorrectie legt een refund uitsluitend als verplichting vast; geldbeweging vereist de aparte MFA-refundactie. Onbekende providerrefunds blokkeren de betaling fail-closed, definitief mislukte providerrefunds tonen geen schijnretry en één handmatige betaling kan meerdere opeenvolgende correctierefundbewijzen dragen.
- De FIFO-prioriteit gebruikt nu het canonieke `settled_at` van de actieve pakketbalans in plaats van de oudste historische betaalrij. Operationele degraded-health blijft releaseblokkerend en zichtbaar, maar verhindert de onafhankelijke schedulerheartbeat niet.
- Afsluitende lokale gates na dit reviewherstel: 60 pgTAP-bestanden/2.040 assertions, uitgebreide package-financeconcurrency, 228 Vitestbestanden/1.409 tests, ESLint, TypeScript, productiebuild en capaciteit groen; capaciteitsp95 staff/QR/uitgifte 423/193/646 ms.
