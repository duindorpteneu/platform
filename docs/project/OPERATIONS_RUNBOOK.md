...
3. Modus `apply` vereist bevestiging `STAGING-CLEANUP-APPLY`, exact origin `https://duindorpsv.dgwebservices.nl`, exact Supabase-project `dxbdjtbyghsovlrdcwcr` en secret `STAGING_CLEANUP_BACKUP_PASSPHRASE`. De bekende production-ref en iedere andere project-ref worden geweigerd.
